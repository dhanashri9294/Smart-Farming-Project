...
def signup_view(request):
    if request.method == "POST":
        ...
        if not name or not email or not phone or not password:
            messages.error(request,"please fi all required fields.")
...
def predict_view(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            messages.error(request,"please enter valid number values.")
            return redirect("predict")
        label = predict_one(data)

        Prediction.objects.create(user=request.user,**data,prediction_label=label)
        result = label
        last_data = data
...
def admin_dashboard_view(request):
    ...
    crop_qs = (
        Prediction.objects.values('prediction_label')
        .annotate( c = Count('id'))
        .order_by('-c')[:10]
    )
    crop_labels = [i['prediction_label'].title() for i in crop_qs]
